...
namespace DNT
{
    public class DNT : Mod
    {
        ...
        static void AddTempVulnerabilities(object[] args, int type)
        {
            ...
            else if (args[1] is Player player)
            {
                var dntPlayer = player.GetModPlayer<DNTPlayer>();

                if (type == 1) dntPlayer.resistances |= elements;
                else if (type == 2) dntPlayer.immunities |= elements;
                else dntPlayer.vulnerabilities |= elements;
            }
            else throw new ArgumentException("(args[1]) Invalid NPC or Player.");
        }
